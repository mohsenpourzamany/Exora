/**
 * Auto-generate unique hawala tracking code
 * Format: HW-[Jalali Year]-[Month]-[Day]-[Daily Sequence]
 * Example: HW-1403-02-24-0001
 *
...
function getTodayJalali(): { year: string; month: string; day: string; key: string } {
  ...
  const key   = `hawala_seq_${year}_${month}_${day}`

  return { year, month, day, key }
}

export function generateHawalaCode(): string {
  ...
  return `HW-${year}-${month}-${day}-${seq}`
}

export function peekNextHawalaCode(): string {
  const { year, month, day, key } = getTodayJalali()
  const stored = localStorage.getItem(key)
  const current = stored ? parseInt(stored, 10) : 0
  const next = current + 1
